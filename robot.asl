...
+!has(owner,beer)
   ...
   <- .send(robotConcierge, achieve, order_beers);
      .print("Conceirge Robot is ordering beers");
...
// Robot concierge restocks the beer in the fridge
+!restocked(beer,_Qtd,_OrderId)[source(robotConcierge)]
  :  true
  <- +available(beer,fridge);
     .print("Beer is stocked in the Fridge again");
     !has(owner,beer).
